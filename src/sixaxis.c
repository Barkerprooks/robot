...
void init_sixaxis() {
    ...
    gpio_set_function(SPI_SDA_PIN, GPIO_FUNC_I2C);   
    gpio_set_function(SPI_SCK_PIN, GPIO_FUNC_I2C); 
    gpio_pull_up(SPI_SDA_PIN);
...
void reset_sixaxis() {
    // 0x6b = reset, 0x80 = write request
    uint8_t reset_bytes[2] = { 0x6b, 0x80 };
